...
module M = struct
  ...
  let parse input =
    ...
    in
    let nums =
      List.map nums ~f:(fun s ->
          List.filter s ~f:(fun l -> String.is_empty l |> not) )
    in
    let left, right =
      List.fold nums ~init:([], []) ~f:(fun (left, right) single ->
          let first, second =
            (List.hd_exn single, List.hd_exn (List.tl_exn single))
          in
  ...
  let part1 (left, right) =
    let left, right =
      Tuple2.map (left, right) ~f:(fun l -> List.sort l ~compare:Int.compare)
    in
    let dists =
  ...
  let part2 (left, right) =
    ...
    List.iter right ~f:(fun x ->
        Hashtbl.update counter x ~f:(function None -> 1 | Some c -> c + 1) ) ;
    let res =
