...
module M = struct
  ...
  let sliding_window ~window_size lst =
    if window_size <= 0 then
      invalid_arg "Window size must be positive"
  ...
  let safe levels = 
    let (res, _) = List.fold (sliding_window ~window_size:2 levels) 
    ~init:(true, 0) ~f:(fun (res, ord) -> function [a; b] -> 
      if not res then (res, ord)
      else (
        let (ord_check, ord2) = (match (ord, Int.compare a b) with
          | (_, 0) -> (false, 0) 
          | (1, -1) -> (false, 0)
          | (-1, 1) -> (false, 0)
  ...
  let part2 reports =
    ...
    let res = List.count ~f:(fun levels ->
      (safe levels) || List.existsi levels ~f:(fun i _ ->
        safe (remove levels i)
        )
    ) reports in
    printf "Part 2: %d\n" res;
    ()
